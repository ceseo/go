...
// func IndexByte(s []byte [R3], s_len [R4], c byte [R5]) int
TEXT bytes·IndexByte(SB),NOSPLIT,$0-40
#ifdef	GOARCH_ppc64le
	// Note: this implementation is currently for LE only because it uses instructions
	// that would break POWER5.
	MOVD	p+0(FP), R3	// R3 = string
	MOVD	b_len+8(FP), R4	// R4 = length
	MOVBZ	c+16(FP), R5	// R5 = byte
	DCBT	(R3)		// Prepare cache line.
	MOVD	R3,R10		// Save base address for later.
	RLDICR	$0,R3,$60,R8	// Align address to doubleword boundary in R8.
	RLDIMI	$8,R5,$48,R5	// Replicating the byte across the register.
	ADD	R4,R3,R7	// Calculate last acceptable address.
	RLDIMI	$16,R5,$32,R5
	CMPU	R4,$32		// Check if it's a small string (<32 bytes). Those will go through the old algorithm.
	MOVD	$-1,R9
	WORD $0x54661EB8	// Calculate padding in R6 (rlwinm r6,r3,3,26,28).
	RLDIMI	$32,R5,$0,R5
	ADD	$-1,R7,R7
	SLD	R6,R9,R9	// Note: if this is ported to BE, just use SRD instead here.
	...
	CMPWU	R3,$0,CR7	// If we have a match, jump to the final computation
	BNE	CR7,done
...
loop_setup:
	// We are now aligned to a 32-byte boundary. We will load two doublewords
...
found:
	...
	ADD	$-1,R3,R11
	ANDN	R3,R11,R11
	POPCNTD	R11,R11		// Count trailing zeros.
	WORD $0x7c283840	// Check if we are at the last doubleword.
	RLDICL	$60,R11,$3,R11	// Convert trailing zeros to bytes.
	ADD	R8,R11,R3
...
TEXT bytes·IndexByte2(SB),NOSPLIT,$0-40
	MOVD	s+0(FP), R3
	MOVD	s_len+8(FP), R4
	MOVBZ	c+24(FP), R5	// byte to find
	MOVD	R3, R6		// store base for later
...
// func IndexByte(p string [R3], b_len [R4], c byte [R5]) int
TEXT strings·IndexByte3(SB),NOSPLIT,$0-32
#ifdef	GOARCH_ppc64le
	// Note: this implementation is currently for LE only because it uses instructions
	// that would break POWER5.
	MOVD	p+0(FP), R3	// R3 = string
