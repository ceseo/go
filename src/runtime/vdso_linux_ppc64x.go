...
const (
	...
	_STT_NOTYPE = 0 /* Symbol has no type */
...
var linux2615 = version_key{"LINUX_2.6.15", 0x75fcba5}

var sym_keys = []symbol_key{
	{"__kernel_time", 0xf876715, &__kernel_time_sym},
	{"__kernel_gettimeofday", 0x7b05f09, &__kernel_gettimeofday_sym},
	{"__kernel_clock_gettime", 0xb0cd725, &__kernel_clock_gettime_sym},
}

var (
	__kernel_time_sym          uintptr = 0
	__kernel_gettimeofday_sym  uintptr = 0
	__kernel_clock_gettime_sym uintptr = 0
)
...
func vdso_parse_symbols(info *vdso_info, version int32) {
	...
	for _, k := range sym_keys {
		for chain := info.bucket[k.sym_hash%uint32(len(info.bucket))]; chain != 0; chain = info.chain[chain] {
			...
			if typ != _STT_NOTYPE || bind != _STB_GLOBAL || sym.st_shndx == _SHN_UNDEF {
				continue
			}
...
func archauxv(tag, val uintptr) {
	...
	case _AT_SYSINFO_EHDR:
		...
		vdso_parse_symbols(info1, vdso_find_version(info1, &linux2615))
	}
}
